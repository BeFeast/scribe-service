...
# --- Cost-aware + stuck-initializing reaping (#355) ---
...
def test_reap_reason_predicate_cost_takes_priority():
    ...
    # Disabled stuck threshold (0) suppresses stuck reaping: a stuck
    # instance that is also old falls through to age.
    old_stuck = {
        "id": 7,
        "label": "x-scribe-whisper-20260527T090000Z",
        "actual_status": "loading",
    ...
    assert (
        vast_reaper.reap_reason_for(
            ...
            baseline_usd_per_hour=baseline,
            cost_multiplier=10.0,
            stuck_threshold=dt.timedelta(0),
        )
        == "age"
    )
