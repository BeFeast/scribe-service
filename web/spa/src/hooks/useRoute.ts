...
export type RoutePage = "library" | "transcript" | "queue" | "job" | "ops" | "settings";
...
type RouteAction = { type: "navigate"; route: Route } | { type: "sync"; route: Route };
...
function routeToHash(route: Route): string {
  ...
  if ((route.page === "transcript" || route.page === "job") && route.params.id !== undefined) {
...
export function useRoute() {
  const [route, dispatch] = React.useReducer(routeReducer, DEFAULT_ROUTE, () => routeFromHash(window.location.hash));

  React.useEffect(() => {
    const sync = () => dispatch({ type: "sync", route: routeFromHash(window.location.hash) });
