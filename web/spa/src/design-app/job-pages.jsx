      ...
      {STAGE_ORDER.map((s, i) => {
        ...
        return (
          <div key={s} className={"stage " + st.state}>
            ...
            {compact && st.note && <div className="stage-note">{st.note}</div>}
...
export function LogTail({ log }) {
  const lines = (log?.lines ?? []).map(adaptLogLine);
  const status = log?.error
    ? { color: "var(--err)", text: log.error }
    : log?.connected
      ? { color: "var(--accent)", text: lines.length === 0 ? "tailing live stream… (no log lines yet)" : "tailing live stream…" }
      : { color: "var(--muted)", text: "waiting for log stream…" };
  return (
    <div style={{
      ...
      <div style={{color: status.color, marginTop: 4}}>
        <span style={{color: "var(--muted)"}}>{currentT()}</span>{"  "}
        <span className="live-dot"/> {status.text}
      </div>
    </div>
  );
}
